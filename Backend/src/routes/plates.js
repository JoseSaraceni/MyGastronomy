...
platesRouter.get("/avaliables/", async (req, res) => {
  const { sucess, statusCode, body } = await platesControllers.getAvaliablePlates();
  res.status(statusCode).send({ sucess, statusCode, body });
});

platesRouter.post("/", async (req, res) => {
  const { sucess, statusCode, body } = await platesControllers.addPlate(
    req.body
  );
  res.status(statusCode).send({ sucess, statusCode, body });
});
...
platesRouter.delete("/:id", async (req, res) => {
  const { sucess, statusCode, body } = await platesControllers.deletePlate(
    req.params.id
  );
  res.status(statusCode).send({ sucess, statusCode, body });
});
...
platesRouter.put("/:id", async (req, res) => {
  const { sucess, statusCode, body } = await platesControllers.updatePlate(
    req.params.id,
    req.body
  );
  res.status(statusCode).send({ sucess, statusCode, body });
